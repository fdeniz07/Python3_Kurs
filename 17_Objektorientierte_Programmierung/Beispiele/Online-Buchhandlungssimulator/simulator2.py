...
class Buch:
	"""Repraesentiert ein Buch im Inventar eines Buchladens."""
	...
	def __str__(self):
		"""Gibt das Buch in einer gut lesbaren Form zurueck."""
		return (
			f"{self.titel} von {self.autor} "
			f"({self.kategorie}, {self.preis:.2f} EUR)"
		)
...
class Buchladen:
	...
	def buch_hinzufuegen(self, buch):
		"""Fuegt ein Buch zum Inventar hinzu."""
		if not isinstance(buch, Buch):
			raise TypeError("Es koennen nur Buch-Objekte hinzugefuegt werden.")
		self.inventar.append(buch)

	def nach_kategorie_suchen(self, kategorie):
		"""Gibt alle Buecher der gewuenschten Kategorie zurueck."""
		return [
			buch
	...
	def gesamtpreis_berechnen(self, buchauswahl):
		"""Berechnet den Gesamtpreis einer Liste von Buechern."""
...
def main():
	...
	print("--- Buecher der Kategorie Roman ---")
